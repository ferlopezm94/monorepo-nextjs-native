...
[![Set release version](https://github.com/good-better/starter-typescript/workflows/Set%20release%20version/badge.svg)](https://github.com/good-better/starter-typescript/blob/main/.github/workflows/set-release-version.yml)
...
```
$ git clone https://github.com/good-better/starter-typescript.git
...
- Install `node_modules` again to setup `husky` hooks
...
- **Continuous integration:** Run on each pull request open to either `main` or `staging` branches. It will run eslint, the automated tests and build the project. If there's an error it will fail, ensuring a new change doesn't break the current codebase.
- **Verify pull request base branch:** Run when a pull request to the `main` branch is open. It will verify the base branch is `staging` and will fail if not the case.
- **Set release version:** Run when changes are pushed to `main`. It will run `semantic-release` and sync the `main` branch with `staging`. To enable this workflow in your repository add the `GH_TOKEN` based on a personal access token with repo scope.
